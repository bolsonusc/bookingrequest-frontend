...
const AccountStatus = () => {
  ...
  const updateVisibility = async () => {
    const token = sessionStorage.getItem('token');
    try {
      // Set provider's visibility
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/providers/visibility`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`,
    ...
    <div className='bg-[#0B0C0E] text-white flex flex-col items-center'>
      ...
      { loading ? 
        ...
        <div className='bg-black-950 my-8 mx-6 flex flex-col justify-center w-[90vw]'>
          ...
          <button className='text-[13px] rounded-xl bg-blue-750 p-2 cursor-pointer hover:bg-blue-800 active:bg-blue-900' onClick={updateVisibility}>Save Changes</button>
        </div>
      }
    </div>
  )
